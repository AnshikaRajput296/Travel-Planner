...
.sidebar-tagline {
  font-size: 0.78rem; color: #64748b; letter-spacing: 1px;
    ...
    <div class="hero-banner">
      <div style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:1rem;">
        ...
        <div style="display:flex; flex-direction:column; gap:8px; align-self:center;">
          <div class="agent-pill">✈️ Flight Agent</div>
      ...
      <div style="margin-top:2rem; display:flex; gap:2rem; flex-wrap:wrap;">
        ...
        <div style="color:#64748b; font-size:0.82rem;">
          🛡️ <span style="color:#f59e0b">Guardrails</span> validation
        </div>
        <div style="color:#64748b; font-size:0.82rem;">
...
if generate_btn:
    ...
    if not result.valid:
        ...
        with st.spinner(""):
            ...
            agent_steps = [
                ("✈️ Flight Agent",         "Estimating flights & airlines…"),
...
if plan:
    ...
    # ── Main Tabs ─────────────────────────────────────────────────────────────
    tabs = st.tabs(["📋 Overview", "✈️ Flights", "🏨 Hotels", "📅 Itinerary", "📊 Budget Analytics", "⭐ Recommendations", "💾 Export"])

    # ── Tab 1: Overview ───────────────────────────────────────────────────────
    with tabs[0]:
        st.markdown('<div class="section-title">🗺️ Trip Overview</div>', unsafe_allow_html=True)
        ...
        with oc1:
            ...
            # Mini budget bar
            categories = [
                ("✈️ Flights",        flight_cost,  "#06b6d4"),
                ("🏨 Accommodation",  hotel_cost,   "#8b5cf6"),
                ("🍽️ Food",          food_cost,    "#f59e0b"),
    ...
    # ── Tab 2: Flights ────────────────────────────────────────────────────────
    with tabs[1]:
        st.markdown('<div class="section-title">✈️ Flight Recommendations</div>', unsafe_allow_html=True)
        ...
        if fd:
            ...
            st.markdown(f"\n🗓️ Best Travel Months: **{fd.get('best_travel_months','')}**")
            st.markdown("</div>", unsafe_allow_html=True)
